...
Durante lo sviluppo ho testato il sistema manualmente, principalmente per verificare che funzionasse e per debuggare i vari problemi che ho incontrato. Non ho condotto benchmark formali o misurazioni sistematiche - il focus era far funzionare l'interoperabilità, non ottimizzare le performance.

\bigskip
\noindent\textbf{Ambiente di Test}

I test sono stati tutti eseguiti in ambiente LAN:
...
\item \textbf{Rete}: LAN 1Gbps, latenza trascurabile
\end{itemize}

\bigskip
\noindent\textbf{Cosa Ho Osservato}

\medskip
\noindent\textit{Il setup della chiamata richiede tempo.}
Dal momento in cui clicco "Call" a quando sento l'audio passano tipicamente 2-4 secondi. La maggior parte di questo tempo è \textit{ICE gathering}: il browser deve contattare server STUN esterni per scoprire il proprio IP pubblico e raccogliere candidati. Guardando \texttt{chrome://webrtc-internals/} (strumento diagnostico fondamentale durante lo sviluppo), ho visto che questa fase può durare 1-3 secondi a seconda della connettività.

Le altre operazioni (creazione SDP, comunicazione con RTPEngine, scambio SIP) sono molto più rapide - si completano in decine di millisecondi. Il DTLS handshake per stabilire la connessione cifrata aggiunge un altro mezzo secondo circa.

Se dovessi ottimizzare, implementerei \textit{ICE trickle}: invece di aspettare tutti i candidati, invio l'SDP subito e mando i candidati ICE man mano che arrivano. Ma per un prototipo, aspettare il gathering completo è più semplice da gestire.

\medskip
\noindent\textit{L'audio funziona bene in LAN.}
Una volta stabilita la chiamata, l'audio è chiaro e senza problemi evidenti. Ovviamente in LAN non ci sono packet loss o jitter significativi. Ho verificato su \texttt{chrome://webrtc-internals/} che:

\begin{itemize}
\item Il bitrate riflette correttamente i codec negoziati (Opus ~48kbps variabile per WebRTC, G.711 fisso 64kbps per SIP)
\item Packet loss praticamente nullo in condizioni normali
\item Jitter basso (pochi millisecondi)
\end{itemize}

L'implementazione delle funzionalità di echo cancellation e noise suppression (dopo aver risolto il problema descritto nella sezione Problematiche) ha fatto una differenza notevole - senza, c'era eco fastidiosa.

\medskip
\noindent\textit{RTPEngine fa il suo lavoro silenziosamente.}
La transcodifica SRTP ↔ RTP avviene senza ritardi percepibili. Controllando i log Docker di RTPEngine, si vede che processa i comandi \texttt{offer()}, \texttt{answer()} e \texttt{delete()} molto rapidamente. Non ho mai visto rallentamenti causati da RTPEngine nelle chiamate singole.

Non ho testato scenari con molte chiamate concorrenti - per quello servirebbero script di load testing che non ho avuto tempo di scrivere. Teoricamente RTPEngine supporta migliaia di sessioni, ma il container Docker con 2 vCPU probabilmente saturerebbe ben prima.

\bigskip
\noindent\textbf{Limitazioni Evidenti}

Durante lo sviluppo sono emerse chiaramente alcune limitazioni:

\begin{enumerate}
\item \textbf{ICE gathering è il collo di bottiglia}: Dipende da server STUN esterni, firewall, NAT. In condizioni di rete difficili può fallire completamente o richiedere TURN relay (che aggiunge latenza).

\item \textbf{RTPEngine è un single point of failure}: Se crasha, tutte le chiamate cadono. Non ho implementato ridondanza. In produzione servirebbe clustering con failover automatico.

\item \textbf{Nessuna autenticazione}: Chiunque può registrarsi con qualsiasi username e chiamare chiunque. Va bene per un prototipo in LAN, assolutamente inadeguato per deployment reale.

\item \textbf{Compatibilità solo con mjSIP}: Le trasformazioni SDP (lowercase \texttt{rtp/avp}, riposizionamento \texttt{c=}, normalizzazione username) sono state fatte specificamente per mjSIP. Con altri stack SIP potrebbero servire adattamenti.

\item \textbf{Nessun test di carico}: Non so come si comporta il sistema con 10, 50, 100 chiamate simultanee. Sospetto che RTPEngine con 2 vCPU saturi rapidamente, ma servirebbe misurarlo.

\item \textbf{Solo LAN testato}: In scenari con NAT complessi, firewall aziendali, connessioni mobile, potrebbero emergere problemi che non ho incontrato in LAN.
\end{enumerate}

\bigskip
\noindent\textbf{Riflessioni Finali}

Il sistema funziona - le chiamate si stabiliscono, l'audio è buono, i problemi che ho incontrato (e ce ne sono stati parecchi, vedi sezione Problematiche) sono stati risolti. È un prototipo funzionale che dimostra la fattibilità dell'integrazione WebRTC-SIP.

Però è chiaramente un progetto didattico, non un prodotto. Mancano: autenticazione, test approfonditi, gestione errori robusti, monitoraggio, logging strutturato completo, alta disponibilità, documentazione deployment, e probabilmente altri dieci aspetti che non mi vengono in mente.

Ciò che ho imparato è che WebRTC e SIP, pur essendo entrambi protocolli per VoIP, sono \textit{molto} diversi. Farli parlare richiede più di un semplice "inoltro messaggi" - serve traduzione attiva, normalizzazione, gestione di casi edge (Firefox vs Chrome, mjSIP strict parsing, ritrasmissioni UDP, NAT). Ma è possibile, anche con stack legacy mai pensati per WebRTC.
