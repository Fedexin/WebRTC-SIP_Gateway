...
Il sistema è stato testato in diverse configurazioni per valutarne prestazioni, latenza e scalabilità.
...
\bigskip
\noindent\textbf{Metriche di Performance}

\medskip
\noindent\textit{Latenza di Call Setup.}
La latenza di setup è stata misurata dal momento della richiesta di chiamata fino all'inizio del flusso media. I risultati mostrano una media di 2.3 secondi, con la seguente distribuzione:

\begin{table}[h!]
\centering
\caption{Breakdown della latenza di call setup}
\label{tab:latency_breakdown}
\begin{tabularx}{\textwidth}{@{} >{\raggedright\arraybackslash}X r r @{}}
\toprule
\textbf{Fase} & \textbf{Latenza Media (ms)} & \textbf{\% del Totale} \\
\midrule
WebSocket RTT & 8 & 0.3\% \\
Creazione offer WebRTC & 45 & 2.0\% \\
ICE gathering & 1200 & 52.2\% \\
Elaborazione RTPEngine (offer) & 12 & 0.5\% \\
Trasmissione SIP INVITE & 35 & 1.5\% \\
Risposta SIP (180 + 200 OK) & 180 & 7.8\% \\
Elaborazione RTPEngine (answer) & 10 & 0.4\% \\
Creazione answer WebRTC & 40 & 1.7\% \\
DTLS handshake & 320 & 13.9\% \\
Inizio flusso RTP & 450 & 19.6\% \\
\midrule
...
La fase più costosa è il gathering dei candidati ICE (52\% del tempo totale), seguita dall'inizializzazione del flusso RTP (20\%). Questi tempi sono coerenti con le implementazioni WebRTC standard e sono influenzati principalmente dai timeout di rete e dalla necessità di contattare server STUN esterni.

\medskip
\noindent\textit{Latenza Media.}
Una volta stabilita la chiamata, la latenza end-to-end del flusso audio è stata misurata utilizzando pacchetti di test con timestamp. I risultati mostrano:

\begin{itemize}
\item \textbf{Latenza WebRTC → SIP}: 38ms (media), 52ms (95° percentile)
\item \textbf{Latenza SIP → WebRTC}: 41ms (media), 58ms (95° percentile)
\end{itemize}

La latenza aggiuntiva introdotta da RTPEngine è di circa 5-8ms, un overhead accettabile considerando le operazioni di cifratura/decifratura SRTP e potenziale transcoding codec.

\medskip
\noindent\textit{Qualità Audio.}
L'analisi della qualità audio è stata effettuata tramite \texttt{chrome://webrtc-internals/} monitorando le statistiche RTP:

\begin{table}[h!]
\centering
\caption{Statistiche qualità audio (sessione di 5 minuti)}
...
Jitter medio & 3.2ms & 4.1ms \\
Bitrate medio & 48 kbps (Opus) & 64 kbps (PCMU) \\
Round-trip time (RTT) & 76ms & 78ms \\
\bottomrule
\end{tabularx}
\end{table}

Il packet loss estremamente basso (<0.05\%) e il jitter contenuto (<5ms) indicano un'eccellente qualità della connessione. Il bitrate è conforme ai codec utilizzati: Opus a 48 kbps per WebRTC e G.711 PCMU a 64 kbps per SIP.

\bigskip
\noindent\textbf{Scalabilità}

Per valutare la scalabilità del sistema sono stati effettuati test di carico simulando chiamate concorrenti. RTPEngine è configurato con un range di porte RTP da 10000 a 20000, teoricamente supportando fino a 10000 sessioni simultanee.

\begin{table}[h!]
\centering
\caption{Test di scalabilità}
\label{tab:scalability}
\begin{tabularx}{\textwidth}{@{} r r r r @{}}
\toprule
\textbf{Chiamate Concorrenti} & \textbf{CPU (\%)} & \textbf{RAM (MB)} & \textbf{Setup Time (ms)} \\
\midrule
10 & 12 & 450 & 2280 \\
50 & 34 & 890 & 2410 \\
100 & 58 & 1520 & 2650 \\
...
I risultati mostrano che il sistema scala linearmente fino a circa 100 chiamate concorrenti. Oltre questa soglia, il tempo di setup inizia a degradare a causa della saturazione della CPU del container RTPEngine. In un ambiente di produzione, si raccomanda il deployment di istanze multiple di RTPEngine con load balancing.

\bigskip
\noindent\textbf{Consumo di Banda}

Il consumo di banda per chiamata varia in base ai codec utilizzati:

\begin{itemize}
\item \textbf{Audio only (Opus 48kbps)}: ~60 kbps (con overhead IP/UDP/RTP)
\item \textbf{Audio only (G.711 64kbps)}: ~80 kbps
\item \textbf{Video 720p (VP8)}: ~1.5 Mbps
\end{itemize}

RTPEngine introduce un overhead trascurabile (<2\%) dovuto all'incapsulamento SRTP.

\bigskip
\noindent\textbf{Considerazioni Finali}

I risultati sperimentali dimostrano che l'architettura implementata è capace di gestire efficacemente l'interoperabilità tra WebRTC e SIP, mantenendo latenze contenute e qualità audio elevata. Le principali limitazioni riscontrate riguardano:

\begin{enumerate}
\item \textbf{ICE gathering time}: Il tempo necessario per raccogliere i candidati ICE (1-2 secondi) impatta significativamente il tempo totale di setup. L'utilizzo di un pool pre-allocato di candidati potrebbe ridurre questa latenza.

\item \textbf{Scalabilità verticale}: RTPEngine rappresenta un single point of failure. Per ambienti ad alto traffico, è necessario implementare architetture distribuite con load balancing.

\item \textbf{Codec transcoding}: Quando è necessaria la transcodifica Opus ↔ G.711, il carico computazionale aumenta. L'uso di hardware dedicato o offload GPU potrebbe migliorare le prestazioni.
\end{enumerate}

Nonostante queste limitazioni, il sistema risulta adeguato per applicazioni real-world come call center ibridi, softphone web aziendali e piattaforme di unified communication con traffico moderato (fino a 100 chiamate concorrenti per istanza).
