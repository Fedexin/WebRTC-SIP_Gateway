...
class SipGateway extends EventEmitter {
  ...
  addDTMFFlags(rtpenginePayload) {
    // FIX: Rimuovi parametri DTMF non supportati dalla versione corrente di RTPEngine
    // Usa solo i parametri standard supportati
    return {
      ...rtpenginePayload,
      'codec': {
        'strip': [],
    ...
    try {
      ...
      const inviteMessage = {
        ...
        headers: {
          ...
          'max-forwards': '70',
          'from': `"${this.config.displayName}" <sip:gateway@${this.config.sipDomain}>;tag=${fromTag}`,
          'to': `<${targetSipUri}>`,
          'call-id': callId,
          'cseq': `${cseq} INVITE`,
          'contact': `"${this.config.displayName}" <sip:gateway@${this.advertiseIP}:${this.config.localSipPort}>`,
  ...
  async handleIncomingInvite(request, rinfo, transactionKey) {
    ...
    try {
      ...
      // FIX: Registra la transazione INVITE
      ...
      this.sessions.set(callId, {
        ...
        createdAt: Date.now(),
        ackReceived: false,
        transactionKey // FIX: Salva il transaction key
      });
      ...
      // FIX: Rimuovi la transazione in caso di errore
      if (transactionKey) {
        this.inviteTransactions.delete(transactionKey);
      }
    }
  }
  ...
  async answerSipCall(callId, webrtcUserId, webrtcAnswerSdp) {
    ...
    try {
      ...
      const retransmit200OK = () => {
        ...
        // Verifica se abbiamo raggiunto il massimo di ritrasmissioni
        if (sessionData.retransmit200Count >= sessionData.maxRetransmits) {
          this.logger.warn('Max 200 OK retransmissions reached, waiting for Timer H', {
            callId,
            count: sessionData.retransmit200Count
          });
          return; // Timer H gestirà il timeout finale
        }

        sessionData.retransmit200Count++;
        this.logger.debug('Retransmitting 200 OK', {
          callId,
          attempt: sessionData.retransmit200Count,
          maxRetransmits: sessionData.maxRetransmits
        });

        this.sendResponse(request, 200, 'OK', sessionData.rinfo, answerResponse.sdp);

        // RFC 3261: Raddoppia l'intervallo fino a T2 (4 secondi)
        const nextInterval = Math.min(
          sessionData.retransmit200Interval * 2,
          this.T2
        );
        sessionData.retransmit200Interval = nextInterval;

        // Schedula la prossima ritrasmissione
        sessionData.retransmit200Timer = setTimeout(retransmit200OK, nextInterval);
      };

      // Avvia le ritrasmissioni dopo T1 (500ms)
