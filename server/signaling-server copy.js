      ...
      switch (message.type) {
        ...
        case 'answer':
          ...
          if (sipCallData && sipGateway) {
            ...
            try {
              //FIX: Estrai l'SDP come stringa
              // Il client WebRTC potrebbe inviare un oggetto RTCSessionDescription
              let sdpString;
              if (typeof message.data === 'string') {
                sdpString = message.data;
              } else if (message.data && typeof message.data === 'object') {
                // Se è un oggetto RTCSessionDescription, prendi il campo 'sdp'
                sdpString = message.data.sdp || JSON.stringify(message.data);
              } else {
                throw new Error('Invalid SDP format');
              }

              logger.debug('Extracted SDP string', {
                callId: sipCallData.callId,
                sdpType: typeof message.data,
